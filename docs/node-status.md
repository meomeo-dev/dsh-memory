...
- `calls` 存**已完成**调用数(本进程 usage 累计,单一真相源);读取端展示时加上在飞调用数(运行中的调用也是调用,避免「运行中却 0 calls」的矛盾)。
...
- 每张卡片三段:
  ...
  3. **节点带**:3 行(recall / extract / review),每行五列严格对齐(状态点 | 名称 | 运行指示 | 累计 calls | 最近一次):运行指示恒显示(空闲灰 / 运行中蓝,「运行中 ×2 · 12s」);累计 calls 含在飞调用;最近一次为「3m ago · 812ms」,失败时红色错误文本替代耗时,首次调用进行中显示「进行中…」。
