...
状态页顶部(Team 状态卡之后、统计指标块之上)新增**记忆活动大表**:最近 24 小时、每 15 分钟一格,X 轴为时间、Y 轴为 rules/lessons × domain 组合轴,格子显示该窗口内新写入的记忆条目数。一眼看出「哪类记忆、哪个领域、什么时候」在增长。

## 用户故事

1. 作为用户,我想在状态页顶部看到最近 24 小时记忆写入的时空分布:哪个 15 分钟窗口、哪个 type × domain 组合写入了多少条,以观察抽取活动的节奏与热点。
...
- 窗口 = [now−24h, now),每桶 15 分钟 → 96 桶;窗口外条目忽略。v1 旧条目由迁移回填的 createdAt(文件名日期当天本地零点)也按事实落桶。
...
- **位置**:Team 状态卡之后、统计指标块(metric-grid)之上;单独一张全宽卡,标题「记忆活动 (近 24 小时,每 15 分钟一格)」。
- **X 轴**:96 列(固定格宽,容器横向滚动);顶部刻度行按小时标注(每 4 格一个刻度,本地 `HH` 时间)。
...
- **桶语义**:96 桶、15 分钟粒度、[now−24h, now) 窗口;窗口外与未来时间戳条目忽略(单测锁定边界)。
...
- **布局**:全宽卡、横向滚动、sticky 标签列;42 行 × 96 格恒渲染(空窗口全 0)。
