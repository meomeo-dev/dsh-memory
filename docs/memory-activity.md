...
- **X 轴**:24 列,**格宽自适应撑满卡片**(`table-layout: fixed` + `width: 100%`,24 数据列均分;表格 `min-width: 752px` 保底「176px 标签列 + 24px/格」——窄设备仍横向滚动,自适应行为不变);顶部刻度行按小时标注(每桶一格、一桶一小时,本地 `HH` 时间);刻度跨距由 DTO `bucketMinutes` 派生(桶宽变化时自动跟随,非硬编码)。
- **Y 轴**:42 行 = `rules` 段 21 域 + `lessons` 段 21 域(与 DISPLAY 枚举同序),标签列 `r/Domain` / `l/Domain`(sticky 左侧,固定 176px 完整容纳最长域),超限裁剪 + ellipsis(悬停 title 显示全名)——文字永不绘制越界重叠格子区。
...
- **布局**:全宽卡,表格 100% 宽(fixed 布局 24 数据列均分,桌面撑满卡片);窄设备 min-width 752px 保底横向滚动;sticky 标签列 176px;42 行 × 24 格恒渲染(空窗口全 0)。
